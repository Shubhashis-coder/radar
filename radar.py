...
ax.grid(color='green',alpha=0.5) # grid color
ax.set_rticks(np.linspace(0.0,r_max,5)) # show 5 different distances
ax.set_thetagrids(np.linspace(0.0,180.0,20)) # show 10 angles
...
pols, = ax.plot([],linestyle='solid',marker='+',markerfacecolor = 'red',
                 markeredgecolor='red',markeredgewidth=2,
                 markersize=8.0,alpha=0.9) # dots for radar points
...
while True:
    try:
        ...
        if start_word:
            ...
            if angle % 1 ==0: # update every 5 degrees
